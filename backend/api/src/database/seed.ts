...
/**
 * One account, one role. The app ships with a single administrator who can do
 * everything; there are no other credentials to manage.
 */
const ADMIN_ROLE = { key: "admin", label: "Administrator", permissions: ["*"] };

const ADMIN_USER = {
  email: "jainam@gmail.com",
  name: "Administrator",
  password: process.env.ADMIN_PASSWORD ?? "passABC@123",
};
...
export async function seed(
  ...
  await Role.insertMany([ADMIN_ROLE]);
  counts.roles = 1;
  ...
  // Any other account that ever existed is removed: this app has exactly one
  // set of credentials, and the mobile client signs in with it silently.
  await User.deleteMany({ email: { $ne: ADMIN_USER.email } });

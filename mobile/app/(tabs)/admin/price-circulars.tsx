...
import { api, type PriceCircularDraft } from "../../../lib/api";
import { useAuth } from "../../../lib/auth";
import { Field, Input, PrimaryButton } from "../../../lib/inputs";
import { STATUS_COLOR } from "../../../lib/masterData";
import { theme } from "../../../lib/theme";
import { Card, Empty, ErrorNote, ExportButtons, Loading, Pill, SectionTitle } from "../../../lib/ui";
...
export default function PriceCircularsScreen() {
  ...
  const canPublish = PUBLISHERS.includes(user?.role ?? "");
        ...
        {drafts.map((d) => (
          <Pressable key={d._id} style={styles.row} onPress={() => router.push(`/admin/price-circular/${d._id}` as never)}>
            <View style={styles.rowHead}>
              ...
              <Pill label={d.status.toUpperCase()} color={STATUS_COLOR[d.status] ?? theme.color.unknown} />
            </View>
          ...
          <View key={String(c._id)} style={styles.row}>
            <View style={styles.rowHead}>
              ...
              <Pill
                ...
                color={c.status === "active" ? theme.color.leading : theme.color.textFaint}
              />
            </View>
            <Text style={styles.rowMeta}>
